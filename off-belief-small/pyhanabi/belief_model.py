...
class ARBeliefModel(torch.jit.ScriptModule):
    def __init__(
        ...
        self.emb = nn.Linear(10, self.hid_dim // 8, bias=False)
    ...
    def forward(self, x, ar_card_in):
        ...
        ar_card_in = ar_card_in.view(seq * bsize, self.hand_size, 10)
    ...
    def loss(self, batch, beta=1):
        ...
        v0 = v0.view(v0.size(0), v0.size(1), self.hand_size, 17)[:, :, :, :10]
    ...
    def sample(self, obs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for i in range(self.hand_size):
            ...
            onehot_sample_t = torch.zeros(
                bsize, self.num_sample, 10, device=sample_t.device
            )
